...
MUTATIONS: tuple[Mutation, ...] = (
    ...
    Mutation(PROMISE, "app/agent/loop.py",
             "        if final_text and guard_reason is None and PROMISE_TO_RETURN.search(final_text):",
             "        if False:",
             "tests/test_agent.py",
    ...
    # -- самораскрытие и темы заказчика -------------------------------------
    Mutation(GUARD, "app/agent/loop.py",
             "        if final_text and SELF_DISCLOSURE.search(final_text):",
             "        if False:",
             "tests/test_agent.py",
             "рубеж самораскрытия выключен — «я первой не пишу» уходит клиенту"),
    Mutation(GUARD, "app/agent/loop.py",
             r"\b(?:я\s+)?не\s+пишу\s+перв\w*|\bперв\w*\s+не\s+пишу|",
             "",
             "tests/test_agent.py",
             "«я не пишу первой» выпало из словаря самораскрытия"),
    Mutation(GUARD, "app/agent/loop.py",
             r"\b(?:менеджер|оператор|он|она|они)\s+(?:вам\s+)?отв[еи]т\w*|",
             "",
             "tests/test_agent.py",
             "«он ответит лично» снова не считается обещанием"),
    Mutation(GUARD, "app/agent/loop.py",
             '        if topic and "escalate_to_human" not in tool_calls:',
             "        if False:",
             "tests/test_agent.py",
             "вопрос про животных снова виснет: карточка оператору не уходит"),
    Mutation(GUARD, "app/agent/loop.py",
             "                if re.search(pattern, text, re.IGNORECASE):",
             "                if False:",
             "tests/test_agent.py",
             "темы эскалации из базы знаний перестали опознаваться"),
    Mutation(GUARD, "app/kb/catalog.yaml",
             r"|щен(?:ок|к\\w*|ят\\w*)|",
             "|",
             "tests/test_agent.py",
             "из темы про животных выпали слова — «с 4 собаками» ещё ловится, «с котом» уже нет"),

    # -- единый адрес -------------------------------------------------------
    Mutation(WIRING, "app/kb/catalog.yaml",
             'answer: "д. Тупиково, территория базы отдыха «Чайка», 1, стр. 2. Адрес единый для всех зон."',
             'answer: "Троицк, деревня Тупиково, база отдыха «Чайка», 1, строение 2. Адрес единый для всех зон."',
             "tests/test_kb.py",
             "адрес в базе знаний снова звучит иначе, чем в промте"),
