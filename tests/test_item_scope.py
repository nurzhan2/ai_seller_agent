...
# --------------------------------------------------------------------------
# Падежные формы — аудит 2026-08-29: скормили ALLOW_TITLE_RE/DENY_TITLE_RE
# все 55 реальных заголовков аккаунта (scripts/sync_item_scope.py), искали
# заголовки, где смысловое слово встречается в другом падеже, а текущий
# regex его не ловит. РЕЗУЛЬТАТ: ни одного такого случая на реальных данных
# — "баня"/"банный" вместе как substring-подстроки покрывают все падежи,
# которые реально встречаются в заголовках аккаунта (баня, бане, баню —
# все начинаются с "бан", как и "банный"/"банное"/"банную"), то же для
# "беседка"→беседки/беседке и "купель"→купелью. Переход на стемы (бан,
# беседк, купел, ...) НЕ изменил классификацию НИ ОДНОГО из 55 реальных
# заголовков — предложенный переход не нужен для решения проблемы, которой
# сегодня нет, и добавляет риск: короткие стемы вроде "бан"/"шат"/"юрт"
# ловят больше случайных совпадений в БУДУЩИХ заголовках, чем полные слова.
#
# "банного" (родительный падеж) НЕ матчит "банный" — это правда (см.
# app/config.py:DEFAULT_BLOCKED_ITEMS, коммит 522028c), но это было
# СОБСТВЕННОЕ упущение в примере теста этого файла, а не реальный заголовок
# аккаунта: ни один из 55 живых объявлений не содержит "банного" в такой
# форме. Тесты ниже фиксируют падежи, которые РЕАЛЬНО встречаются, — чтобы
# будущая правка регулярки не сломала то, что сегодня работает.
# --------------------------------------------------------------------------

@pytest.mark.parametrize(
    "title",
    [
        "Аренда теплого гриль-домика и беседки в Троицке",   # беседки — родительный/винительный
        "Купольная беседка в Ватутинках — уют всегда",        # беседка через "купольная" — прилагательное
        "Семейная баня на дровах с купелью в Троицке, Бронь",  # купелью — творительный
        "Русская баня В Ватутинках — отдохнуть с друзьями",   # баня — именительный, с контекстом
    ],
)
def test_real_case_forms_from_the_account_are_allowed(title):
    """Реальные заголовки аккаунта (scripts/sync_item_scope.py, 2026-08-28) —
    регрессия на случай, если кто-то сузит ALLOW_TITLE_RE до границ слова
    (\\b) или до точных форм и молча сломает то, что сегодня работает."""
    decision, reason = classify_title(title)
    assert decision == ALLOW
    assert reason == "title_matches_allow"


def test_genitive_form_of_banya_is_a_known_uncovered_case():
    """Документирует остаточный пробел, а не защищает от него: "баня" не
    покрывает чистый родительный падеж "бани" без других allow-слов рядом.
    Ни разу не встретилось в реальных 55 заголовках (иначе аудит выше его
    бы поймал) — если когда-нибудь встретится и тест ниже станет падать
    (ALLOW вместо ожидаемого), это сигнал добавить конкретное слово, а не
    повод сейчас переходить на короткие стемы ради гипотетического случая."""
    decision, reason = classify_title("Стоимость аренды бани на выходные")
    assert (decision, reason) == (ALLOW, "no_keyword_match")
