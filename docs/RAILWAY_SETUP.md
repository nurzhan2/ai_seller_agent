...
| `AVITO_ALLOWED_ITEMS` | id объявлений через запятую, по которым агенту разрешено отвечать. Список актуальных id: `python -m scripts.export_listings`. **Пусто = разрешено всё** — фильтра нет, агент отвечает по любому объявлению аккаунта, включая вакансии и продажу бизнеса |
