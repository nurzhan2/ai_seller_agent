...
фиксирует это явно (`builder = "dockerfile"`) вместе со `startCommand`
(миграции перед стартом) и healthcheck — трогать не нужно, если не меняете
инфраструктуру осознанно.
