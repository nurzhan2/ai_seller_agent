...
| `AVITO_BLOCKED_ITEMS` | id объявлений через запятую, по которым агент НЕ отвечает. Можно не задавать: пять посторонних объявлений заказчика (вакансия, продажа глэмпинга, арендный бизнес, продажа банного комплекса, квартира-студия) зашиты значением по умолчанию. Новое объявление комплекса работает сразу, без правки переменной |
