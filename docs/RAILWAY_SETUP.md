...
### Первый запуск — обязательно с `POLLER_BACKFILL_HOURS=0`

Поллер видит **весь ящик**, а не только новые сообщения. При ненулевом окне
он на первом же проходе ответит всем, у кого последнее сообщение входящее и
свежее окна: на живом аккаунте в окне 72 часов таких набралось 6 чатов, но
глобального суточного лимита исходящих ещё нет.

С нулём поллер молча помечает все существующие чаты прочитанными и отвечает
только на то, что придёт **после** запуска. Кого разбудить из старого —
решается руками в `/admin/dialogs`, раздел «Пропущено при холодном старте».

### Что проверить перед включением

```bash
python -m scripts.poll_once --dry --backfill-hours 72
```

Читает и ничего не отправляет. Показывает, сколько чатов в аккаунте, кого
поллер обработал бы и по какой причине пропустил бы остальных. Текст
сообщений не печатается ни в одном режиме.
