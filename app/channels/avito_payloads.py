...
# Идентификатор, под которым Авито пишет в чат сам: «пользователь создал чат,
# но пока ничего не написал», «посмотрел номер из объявления», «кандидат
# откликнулся на вакансию».
#
# ПОДТВЕРЖДЕНО ЖИВЫМ ЗАПРОСОМ, а не спеком (конверт сообщений в OpenAPI
# описан слабо, см. докстринг модуля): 2026-09-18 `scripts/poll_once --chat`
# по двум боевым чатам с такими сообщениями — у каждого `author_id=0` и
# `type='system'`, ни у одного сообщения клиента или нашего — ни того, ни
# другого.
SYSTEM_AUTHOR_ID = "0"


def is_system_message(payload: dict) -> bool:
    """Сообщение написал сам Авито, а не человек.

    ПО ОТПРАВИТЕЛЮ, НЕ ПО ТЕКСТУ — требование заказчика. Текст у таких
    сообщений начинается с «[Системное сообщение]», но это оформление Авито:
    его могут поменять в любой день, и тогда фильтр по тексту молча
    перестанет работать, а бот снова начнёт отвечать системе «передала
    менеджеру».

    Два признака через ИЛИ: оба наблюдались вместе, но если однажды Авито
    пришлёт только один из них, пропустить системное сообщение в агента
    хуже, чем лишний раз промолчать на странный конверт.
    """
    if extract_author_id(payload) == SYSTEM_AUTHOR_ID:
        return True
    return extract_message_type(payload) == "system"
