...
(app/config.py) — пятью id, зашитыми в код. Работало, но каждое НОВОЕ
...
ПОРЯДОК ПРОВЕРОК В `ItemScopeResolver.resolve` — ЖЁСТКИЙ DENY ПЕРВЫМ:
  ...
  2. Пять зашитых id (`settings.avito_blocked_items`) — DENY ВСЕГДА, даже
     если заголовок матчит allow-слова. Проверено живьём: объявление
     «Продажа банного комплекса» (7980739861) содержит слово «банный» из
     allow-списка ниже — чистая классификация по ключевым словам открыла бы
     его, а это ПРОДАЖА ВСЕГО КОМПЛЕКСА, не бронирование конкретной зоны.
     Проверяется ПЕРЕД таблицей `item_scope`, а не через закешированную
     строку в ней: список читается из настроек заново на каждый вызов,
     чтобы правка `AVITO_BLOCKED_ITEMS` подействовала сразу, а не после
     следующего часового прохода классификатора.
...
# Посторонние для комплекса объявления заказчика — та же формулировка, что
# была у DEFAULT_BLOCKED_ITEMS (app/config.py) до появления этого модуля.
_DENY_WORDS = (
    "вакансия", "продам", "аренда бизнеса", "квартира", "студия", "глэмпинг",
)
...
def classify_title(title: Optional[str]) -> tuple[str, str]:
    ...
    DENY ПРОВЕРЯЕТСЯ ПЕРВЫМ И ПОБЕЖДАЕТ: заголовок вроде «Продажа банного
    комплекса, участок 15 соток» матчит и allow-слово «банный», и deny-слово
    отсутствует в этом конкретном примере намеренно — жёсткий deny по id
    (см. докстринг модуля) существует именно на случай, когда оба списка
    молчат, а объявление всё равно не про бронирование зоны. Здесь же —
    только заголовки, где идёт речь и о «квартире», и о «бане» одновременно
    (реальный случай на этом аккаунте не встречался, но правило должно быть
    предсказуемым без взгляда на конкретный текст).
