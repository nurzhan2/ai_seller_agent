...
class MessagePipeline:
    ...
    async def _handle_message(self, payload: dict, *, source: str = "webhook") -> bool:
        ...
        if not await dedup.claim(message_id, self._redis):
            ...
            # True, А НЕ False — ЭТО ШТАТНЫЙ ИСХОД, А НЕ СБОЙ. Дубль означает,
            # что сообщение уже обработано (или его прямо сейчас обрабатывает
            # другой воркер), и поллеру нужно двигать курсор дальше — ровно
            # так, как обещает докстринг handle_message выше.
            #
            # Пока здесь стоял False, поллер читал его как «конвейер упал» и
            # курсор НЕ двигал (app/avito/poller.py). Получалось замыкание:
            # холодный старт подтверждал заявку на сутки
            # (WEBHOOK_IDEMPOTENCY_TTL_SECONDS, дефолт 24 ч), курсор за
            # сообщением не закреплялся, и каждый следующий проход упирался
            # в тот же message_id — «чат брошен до следующего прохода» по
            # кругу каждые ~70 с. Найдено на запуске 2026-09-01: 18 чатов,
            # 129 таких пар строк в логе за четверть часа.
            #
            # Хуже холостых запросов было другое следствие. Сообщения новее
            # курсора отдаются по возрастанию, застрявшее — первое; поллер
            # возвращался на нём и до НОВОГО сообщения клиента не доходил.
            # Эти чаты были глухи, пока не истечёт суточная заявка.
            #
            # Оговорка про «прямо сейчас обрабатывает другой воркер»
            # (минутная заявка, PROVISIONAL_TTL_SECONDS): курсор сдвинется,
            # хотя ход ещё идёт. Это безопасно, потому что проход поллера
            # держит распределённый захват (AvitoPoller._acquire_lock) и
            # второго читателя той же очереди нет, а вебхук приходит своим
            # путём и сам сохранит сообщение.
            return True
