...
ПУСТОЙ СПИСОК — ШТАТНОЕ СОСТОЯНИЕ, а не сбой. Импорт 2026-09-18 загрузил
67 файлов по девяти зонам; у домика для отдыха (house_relax) фото в выгрузке
заказчика не было вовсе, и его список пуст. Инструмент на пустой список
отвечает тем же честным «фотографий нет», что и при полностью отсутствующем
провайдере, — агент не должен обещать клиенту то, чего не придёт.

Провайдер отдаёт только id. Путь до клиента: get_photos отбирает до трёх
ещё не показанных (app/agent/tools.py), ход несёт их в TurnResult.photos,
конвейер шлёт после текста через гейт исходящих (app/pipeline.py:
_send_photos) и помечает отправленные в `messages.image_ids`.
"""
