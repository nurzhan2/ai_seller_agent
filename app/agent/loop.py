...
#
# Агент первым не пишет (TOUCH_ENABLED=false, дефолт в коде — см.
# app/config.py), значит «я вернусь с ответом» он исполнить не может: чтобы
# он заговорил снова, клиент обязан написать сам. «Менеджер свяжется» тоже
# запрещено заказчиком 2026-09-06 — клиент садится ждать звонка, которого
# может не быть, и уходит.
#
# Ловим ОБЕЩАНИЕ ОТ ПЕРВОГО ЛИЦА И ОТ ЛИЦА КОМПАНИИ, но не повелительное
# наклонение в адрес клиента: «напишите», «перезвоните», «свяжитесь с нами»
# — это как раз то, чем такие фразы заменяются, и глушить их нельзя.
PROMISE_TO_RETURN = re.compile(
    r"\bверн[уё]сь\b|\bвернемся\b|\bвернёмся\b|"
    r"\bсвяж[уе]сь\b|\bсвяжемся\b|\bсвяжётся\b|\bсвяжется\b|\bсвяжутся\b|"
    r"\bперезвон(?:ю|им|ит|ят)\b|"
    r"\b(?:напишу|напишем|отпишусь|отпишемся|сообщу|сообщим|уведомлю)\b\s+"
    r"(?:вам|тебе|позже|как\s+только|сразу)|"
    r"\bда[мс]?\s+знать\b|\bдадим\s+знать\b|"
    # «он ответит лично» — то же обещание чужими словами. Найдено в проде
    # 2026-09-07: прежний набор ловил «свяжется», но не «ответит».
    r"\b(?:менеджер|оператор|он|она|они)\s+(?:вам\s+)?отв[еи]т\w*|"
    r"\bответ(?:ит|ят)\s+(?:вам|лично|позже)",
    re.IGNORECASE,
)
# КЛИЕНТУ НЕЛЬЗЯ РАССКАЗЫВАТЬ, КАК УСТРОЕН АГЕНТ.
#
# Прод 2026-09-07, дословно: «Про собак уже передала вопрос менеджеру — он
# ответит лично, Я ПЕРВОЙ НЕ ПИШУ». Клиент получил кусок нашей внутренней
# механики вместо ответа. Источник нашёлся сразу: правило №6 системного
# промта объясняло модели, ПОЧЕМУ нельзя обещать вернуться («Ты не пишешь
# первым: чтобы разговор продолжился, клиент должен написать сам»), — и
# модель пересказала объяснение клиенту. Объяснение из промта убрано, но
# полагаться на это одно нельзя: модель способна сочинить такую фразу и без
# подсказки.
#
# Ловим три класса: «я бот/ассистент/ИИ», «мне не разрешено / я не пишу
# первой / у меня нет доступа» и «передам в систему / система не позволяет».
SELF_DISCLOSURE = re.compile(
    r"\bя\s+(?:бот|робот|ассистент\w*|нейросет\w*|программ\w*|ии\b|(?:чат-?)?gpt)|"
    r"\bя\s+не\s+(?:человек|живой|настоящ\w*)|"
    r"\b(?:я\s+)?не\s+пишу\s+перв\w*|\bперв\w*\s+не\s+пишу|"
    r"\bмне\s+(?:не\s+)?(?:разрешено|запрещено|нельзя\s+самой)|"
    r"\bу\s+меня\s+нет\s+(?:доступа|прав|возможности\s+написать)|"
    r"\bмо[яи]\s+(?:инструкц\w*|настройк\w*|систем\w*)\b|"
    r"\bпереда[мю]\s+в\s+систему|\bсистема\s+не\s+(?:позволяет|даёт|дает)|"
    r"\bпо\s+инструкции\s+я\b",
    re.IGNORECASE,
)
SELF_DISCLOSURE_VIOLATION = (
    "самораскрытие: агент рассказал клиенту, как он устроен (последний рубеж)"
)

PROMISE_GUARD_VIOLATION = (
    "обещание вернуться или перезвонить (последний рубеж): агент первым не пишет"
)
...
#
# «ЗАНЯТЬСЯ» — НЕ «ЗАНЯТО». Замер 2026-09-18 (scripts/probe_questions.py,
# случай client_ignored_the_question): на вопрос «а у вас есть где детям
# поиграть?» модель трижды из пяти ответила честно — «да, есть детская
# площадка, детям будет чем заняться», — а рубеж срезал ответ целиком и
# подставил «на сколько часов планируете?». Клиенту, задавшему вопрос,
# ответили вопросом: ровно тот допрос, на который жаловался заказчик.
# Причина — основа `занят\w*`, накрывавшая и «заняться», и «занятие».
# Третий прогон того же замера нашёл «будет чем занять ребят» — исключён
# и инфинитив «занять» (он же начало «заняться»). Исключены эти лексемы; «занят/занята/занято/заняты/занятость»
# ловятся по-прежнему.
AVAILABILITY_WORDS = re.compile(
    r"\bсвобод(?:ен|ны|на|но|н\w*)|\bосвобод\w*|\bосвобожд\w*|\bпосвободн\w*|"
    r"\bзанят(?!ь|ие|ия|ий|ии|иям|иями|иях|ием|н)\w*|"
    r"\bнезанят\w*|\bнесвободн\w*|\bнесвобод(?:ен|на|но|ны)\b|"
...
AVAILABILITY_GUARD_HANDOFF = (
    ...
    # попадать под его же словарь — иначе она блокирует себя. Держится
    # тестом test_the_guard_replies_do_not_trip_the_guard_itself.
    "С календарём разбирается менеджер — я передала ему ваш вопрос. "
...
# СЛЕДУЮЩИЙ ХОД ОСТАЁТСЯ ЗА КЛИЕНТОМ. Агент первым не пишет
# (TOUCH_ENABLED=false), поэтому «вернусь с ответом» — обещание, которое он
# физически исполнить не может. «Менеджер свяжется» — не лучше: заказчик
# 2026-09-06 потребовал убрать и его, потому что клиент садится ждать
# звонка, которого может не быть. Формулировка ниже не обещает ничего и
# оставляет ход клиенту.
HANDED_TO_MANAGER = "Передала вопрос менеджеру — напишите, если нужно что-то ещё."
GUARD_RAIL_FALLBACK = (
    "Это уточняет менеджер — я передала ему вопрос. "
    "Напишите, если нужно что-то ещё."
)
...
def escalation_topic_in(client_text: str, kb: Any) -> Optional[str]:
    """Тема из `escalation_topics`, если она есть в сообщении КЛИЕНТА.

    ЗАЧЕМ ЭТО В КОДЕ. Блок `escalation_topics` в app/kb/catalog.yaml лежал с
    самого начала, но код его только разбирал и НИКАК не использовал:
    правило было переписано в системный промт руками. Промт модель исполняет
    через раз — и 2026-09-07 не исполнила. Клиент спросил про четырёх собак,
    модель ответила про вместимость и цену, `escalate_to_human` не вызвала,
    а следующим сообщением написала «уже передала вопрос менеджеру». Карточка
    оператору не уходила: клиент ждал ответа, которого никто не готовил.

    Теперь тема опознаётся ЗДЕСЬ, по словам из самой базы знаний (поле
    `match`), и эскалация ставится независимо от решения модели. Возвращает
    описание темы для журнала либо None.
    """
    text = client_text or ""
    if not text.strip():
        return None
    seen: set[str] = set()
    for zone in getattr(getattr(kb, "catalog", None), "zones", []) or []:
        for topic in getattr(zone, "escalation_topics", None) or []:
            topic_id = str(topic.get("id") or "")
            pattern = topic.get("match")
            if not pattern or topic_id in seen:
                continue
            seen.add(topic_id)
            try:
                if re.search(pattern, text, re.IGNORECASE):
                    return str(topic.get("trigger") or topic_id)
            except re.error:
                # Кривой шаблон в базе знаний не должен ронять ход клиента.
                logger.warning("escalation_topics: не разобрать match у темы %r",
                               topic_id)
    return None
...
class AgentLoop:
    ...
    async def run_turn(
        ...
        if classification == "human":
            ...
            return TurnResult(
                text=not_a_verbatim_repeat(HANDED_TO_MANAGER, history),
                escalated=True,
        ...
        # сочинять календарь.
        # ТЕМЫ, КОТОРЫЕ РЕШАЕТ ЗАКАЗЧИК ЛИЧНО (животные, стол гостям юрты,
        # своя палатка). Ставим эскалацию САМИ, не спрашивая модель: см.
        # escalation_topic_in. Текст ответа при этом не трогаем — модель
        # обычно отвечает «уточню у менеджера», и это верно; не хватало
        # ровно карточки оператору.
        topic = escalation_topic_in(user_text, self.kb)
        if topic and "escalate_to_human" not in tool_calls:
            logger.warning(
                "escalation topic %r в сообщении клиента, а модель не "
                "эскалировала — ставим эскалацию кодом",
                topic, extra={"dialog_id": dialog_id},
            )
            executor.escalated = True
            executor.escalation_reason = (
                f"тема решается заказчиком лично: {topic}"
            )

        guard_reason: Optional[str] = None
        if final_text and SELF_DISCLOSURE.search(final_text):
            # Первым: рассказ о собственном устройстве — не про календарь и
            # не про деньги, остальные рубежи его не видят.
            guard_reason = SELF_DISCLOSURE_VIOLATION
        if final_text and guard_reason is None and PROMISE_TO_RETURN.search(final_text):
            # Отдельной веткой ДО остальных: обещание вернуться — не про
            # календарь и не про деньги, его не поймает ни один другой рубеж.
            guard_reason = PROMISE_GUARD_VIOLATION
        if final_text and guard_reason is None:
