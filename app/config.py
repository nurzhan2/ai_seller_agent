...
class Settings(BaseSettings):
    ...
    #
    # ДЕФОЛТ 0 — защита не активна (не unix-время ни для одного реального
    # сообщения, поэтому `created < 0` никогда не истинно). Стенды без
    # инцидента ничего не меняют. Выставляется вручную: unix-секунды,
    # раньше которых агент отвечать не должен, — например, момент, когда
    # поллер включают обратно. Только пока это значение > 0, отсутствие
    # `created` у входящего тоже трактуется как «слишком старое» (fail
    # closed, тот же принцип, что у OutboundGate.is_allowed и у
    # OwnItemIds.__call__): свежести сообщения, о которой нечего сказать,
    # доверять нельзя.
