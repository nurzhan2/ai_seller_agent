...
class Settings(BaseSettings):
    ...
    # Холодный старт: автоответ только там, где последнее сообщение входящее
    # и свежее этого окна. Всё остальное помечается прочитанным БЕЗ единого
    # исходящего и попадает списком в /admin/dialogs.
    #
    # ДЕФОЛТ 0 — «не будить никого». Первый запуск обязан пройти вхолостую:
    # на живом аккаунте в окне 72 часов набралось 6 чатов, но глобального
    # суточного лимита исходящих ещё нет (он в блоке 4), и ошибка здесь
    # стоит десятков одновременных сообщений живым людям. Поднимать значение
    # осознанно, после того как канал показал, что работает.
    poller_backfill_hours: int = 0
    ...
    # время и оставил контакты. Раньше было False «до стабильных метрик
    # модерации» — метрики набраны, автономная работа включена.
    #
    # Что при этом НЕ отменяется (см. app/agent/tools.py:_tool_create_booking):
    # занятость перепроверяется заново непосредственно перед постановкой,
    # блокируются ЧАСЫ ЗАНЯТОСТИ (при акции «6-й час в подарок» их 6, а не
    # оплаченные 5), бронь пишется в нашу таблицу `bookings`, оператор
    # получает уведомление. Агенту по-прежнему запрещено говорить
    # «забронировал» и «бронь подтверждена» — только «придержала время».
